...
priority_scheduling() {
  ...
  for ((i = 0; i < n; i++)); do
    echo "P$i        ${burst_times[$i]}            ${priorities[$i]}           ${wait_time[$i]}             ${turnaround_time[$i]}"
  done
...
for ((i = 0; i < n; i++)); do
  read -p "Enter priority for process P$i (lower number means higher priority): " priority
  priorities+=($priority)
done

priority_scheduling "$n" "${burst_times[@]}" "${priorities[@]}"
